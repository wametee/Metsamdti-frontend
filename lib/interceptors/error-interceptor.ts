  ...
  httpClient.interceptors.response.use(
    ...
    (error: AxiosError) => {
      ...
      switch (status) {
        ...
        case 401:
          // For login requests, use the backend's error message (which is "Wrong email or password")
          // For other requests, show generic unauthorized message
          const isLoginRequest = error.config?.url?.includes('/auth/login') || error.config?.url?.includes('/login');
          const loginErrorMessage = isLoginRequest 
